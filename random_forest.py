...
#    np.savetxt('result_rand/result'+str(i)+'.csv', pred, fmt='%10.5f', delimiter=',') 
#    
#i = 8
#X = df.iloc[i*100:, :299]
#y = df.iloc[i*100:, 300]
...
for vec in list_pred:
    ...
    while(pred.size < 100):
        pred = np.append(pred, 2.0)
    np.savetxt('result_rand_pred/result'+str(i)+'.csv', pred, fmt='%10.5f', delimiter=',') 
    i += 1 
